...
If a "Good" agent starts failing, the system detects the "Drift" and statistically ejects them from the "High Performer" cluster, causing their utility score to drop precipitously before they can inflict widespread damage.
...
Layer	Function	Defense Against
Graph-Aware Layer	Analyzes flows of trust between users.	Collusion & Sybil Cliques: Detects if a group of bots are only reviewing each other.
Economic Layer	Incorporates "Skin-in-the-Game" (Staking).	Spam: Makes it financially irrational to create thousands of fake agents.
Semantic Layer	Translates math into Fuzzy Logic.	Usability: Converts complex vectors into human-readable badges like "Verified Anchor" or "Volatile."
Anomaly Layer	Bio-inspired "Immune System."	Zero-Day Exploits: Freezes trust updates if an agent exhibits completely novel, non-human behavior patterns.
